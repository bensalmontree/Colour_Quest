...
class ChooseRounds:
    ...
    def __init__(self):
        ...
        self.intro_heading_label = Label(self.intro_frame, text="Colour Quest", font=("Arial", "16", "bold"))
        self.intro_heading_label.grid(row=0)

        choose_instructions_txt = "In each round you will be given six different colours to choose from.  Pick a colour and see if you can beat the computer's score!\n\n To begin, choose how many rounds you'd like to play..."
        ...
        # list to set up rounds button.  First item in each sublist is the background color, second item is the number of rounds
        btn_color_value = [
            ["#CC0000", 3], ["#009900", 5], ["#000099", 10]
        ]

        for item in range(0, 3):
            self.rounds_button = Button(self.how_many_frame, fg=button_fg, bg=btn_color_value[item][0], text="{} Rounds".format(btn_color_value[item][1]), font=button_font, width=10, command=lambda i=item: self.to_play(btn_color_value[i][1]))
...
class Play:
    ...
    def __init__(self, how_many):
        ...
        # list to hold references for coloured buttons so that they can be configured for new rounds etc
        self.choice_button_ref = []

        for item in range(0, 6):
            self.choice_button = Button(self.choice_frame, width=15, command=lambda i=item: self.to_compare(self.button_colours_list[i]))
        ...
        self.comp_choice_label = Label(self.quest_frame, text="Computer Choice will appear here", bg="#C0C0C0", width=51)
        ...
        self.round_results_label = Label(self.rounds_frame, text="Round 1: User: - \t Computer: -", width=32, bg="#FFF2CC", font=("Arial", 10), pady=5)
        ...
        # list to hold references for control buttons so that the text of the 'start over' button can easily be configured when the game is over
        ...
        # Access stats and help buttons so that they can be enabled / disabled
        self.to_help_btn = self.control_button_ref[0]
        self.to_stats_btn = self.control_button_ref[1]

        # Disable stats button at start of game (as there are no stats to display)
    ...
    def to_compare(self, user_choice):
        ...
        # get computer choice and add to list for stats when getting score, change it to an integer before appending
        ...
        # if the game is over, disable all buttons and change text of 'next' button to either 'You Win' or 'You Lose' and disable all buttons
...
class DisplayStats:
    ...
    # calculate total, best, worst and average score from list of scores.
    @staticmethod
